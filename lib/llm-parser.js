...
function buildPrompt(rawText, parserConfig) {
  ...
  // Strip images and attachment markdown before sending to LLM (keep original body later)
  const mediaList = [];
  let cleanText = rawText
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (full, alt, url) => {
      mediaList.push({ type: 'image', full, alt, url });
      return `[图片：${alt || '截图'}]`;
    })
    .replace(/\[([^\]]+)\]\((\/[^\s)]+)\)/g, (full, name, url) => {
      mediaList.push({ type: 'file', full, name, url });
      return `[附件：${name || '文件'}]`;
    })
    .replace(/(https?:\/\/[^\s　）"\')>）\]]+)/g, (full) => {
      // Also capture plain http/https links not in markdown format
      if (!mediaList.some(m => m.url === full)) {
        mediaList.push({ type: 'link', full, alt: full, url: full });
      }
      return `[链接：${full}]`;
    });

  // Truncate extremely long text to avoid timeout (keep first 2500 chars)
  // Use Array.from to safely handle Unicode surrogate pairs
  // Also ensure we don't cut URLs in the middle (URL regex match)
  const MAX_TEXT = 2500;
  if (cleanText.length <= MAX_TEXT) {
    text = cleanText;
  } else {
    // Find the last URL boundary before MAX_TEXT to avoid truncating URLs
    const urlPattern = /https?:\/\/[^\s　）"\')>）\]]+/g;
    let cutoff = MAX_TEXT;
    let match;
    // Find all URL matches and check if any extends past MAX_TEXT
    const urlMatches = [];
    while ((match = urlPattern.exec(cleanText)) !== null) {
      urlMatches.push({ start: match.index, end: match.index + match[0].length });
    }
    for (const m of urlMatches) {
      if (m.start < MAX_TEXT && m.end > MAX_TEXT) {
        // A URL spans the cutoff - extend cutoff to end of URL
        cutoff = Math.max(cutoff, m.end);
      }
    }
    text = cleanText.slice(0, cutoff) + '\n...(文本过长已截断)';
  }
...
// Sanitize string: remove Unicode replacement characters and control chars
...
function validateNotice(obj, idx) {
  ...
  notice.title = sanitizeStr(obj.title) || '通知';
  notice.title = notice.title.substring(0, 80);

  // body: string, required
  notice.body = sanitizeStr(obj.body || obj.title || '');
  ...
  // owner: string, default "未指定". Filter out hallucinated Chinese names from email pinyin
  let owner = sanitizeStr(obj.owner);
  // If owner is purely Chinese name (2-3 chars) that doesn't appear anywhere in the body,
  // it's likely a hallucination from pinyin → revert to "未指定"
  if (/^[\u4e00-\u9fa5]{2,3}$/.test(owner)) {
    const body = String(obj.body || '');
    if (!body.includes(owner)) owner = ''; // hallucinated, discard
  }
  notice.owner = owner || '未指定';

  // location: string or null
  notice.location = obj.location ? sanitizeStr(obj.location) : null;
  ...
  notice.keyPoints = (Array.isArray(obj.keyPoints) ? obj.keyPoints : [])
    .map(k => sanitizeStr(k).substring(0, 100))
    .filter(k => k.length > 3)
    .slice(0, 5);
...
async function parseWithLLM(rawText, parserConfig) {
  ...
  const { prompt, mediaList } = buildPrompt(rawText, parserConfig);
  ...
  // Restore images/attachments: LLM may strip them, always append original media to body
  if (mediaList && mediaList.length > 0) {
    for (const n of validated) {
      for (const m of mediaList) {
        if (m.type === 'link') {
          // For plain URLs: restore placeholder -> real URL, and add to links array if missing
          const placeholder = `[链接：${m.url}]`;
          if (n.body.includes(placeholder)) {
            n.body = n.body.replace(placeholder, m.url);
          }
          if (!n.links.includes(m.url)) {
            n.links.push(m.url);
          }
        } else if (!n.body.includes(m.full)) {
          // For images and local files: append to body if not present
          n.body = n.body + '\n' + m.full;
        }
      }
      // Clean placeholders from keyPoints
      for (let i = 0; i < n.keyPoints.length; i++) {
        n.keyPoints[i] = n.keyPoints[i].replace(/\[图片[^\]]*\]/g, '').replace(/\[附件[^\]]*\]/g, '').replace(/\[链接：[^\]]*\]/g, '').trim();
      }
    }
  }
