...
// 确保目录存在
function ensureDirs() {
  [DATA_DIR, UPLOAD_DIR, IMAGES_DIR, ATTACHMENTS_DIR].forEach(dir => {
...
async function uploadToStorage(fileContent, cloudPath, savedName, type) {
  ...
  return {
    url: `${process.env.BASE_URL || 'http://localhost:3000'}/uploads/${type === 'image' ? 'images' : 'attachments'}/${savedName}`,
    name: savedName,
    size: fileContent.length,
    cloudPath
  };
