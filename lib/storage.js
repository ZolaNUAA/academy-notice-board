...
async function uploadToStorage(fileContent, cloudPath, savedName, type) {
  ...
  // 方式1: 腾讯云容器环境 -> 使用 CloudBase storage（优先）
  if (isOnCloudBase()) {
    try {
      ...
      if (urlResult.fileList && urlResult.fileList[0] && urlResult.fileList[0].tempFileURL) {
        console.log(`[Storage] Saved to CloudBase: ${result.fileID}`);
        return {
  ...
  if (USE_MNT) {
    try {
      ...
      return {
        url: `${normalizeBaseUrl(process.env.BASE_URL)}/data/uploads/${type === 'image' ? 'images' : 'attachments'}/${savedName}`,
        name: savedName,
        size: fileContent.length,
        cloudPath,
        storage: 'mnt'
